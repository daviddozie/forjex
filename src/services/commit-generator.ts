...
export class CommitMessageGenerator {
        ...
        lines.forEach((line, index) => {
            ...
            if (line.startsWith('+')) {
                ...
                if (fileMatch) {
                    ...
                    fileChanges[currentFile] = { importsAdded: [] };
                }

                if (currentFile && line.startsWith('+')) {
                    const importMatch = line.match(/\+\s*import\s+.*from\s+['"]([^'"]+)['"]/);
                    if (importMatch) {
                        fileChanges[currentFile].importsAdded.push(importMatch[1]);
                    }
                }
    ...
    private buildDescription(
        ...
        if (changes.fileChanges) {
            for (const [file, change] of Object.entries(changes.fileChanges)) {
                if (change.importsAdded?.length > 0) {
                    const imports = change.importsAdded.map((i:any) => i.split('/').pop()).join(', ');
                    parts.push(`add ${imports} import in ${file}`);
                }
            }
        }
